...
#### 3. `/backup`
サーバーを抜けたメンバーを検出し、バックアップレポートを生成します。
...
- 認証情報を検証
- 認証済みで退出したメンバーを検出
- サーバー内のメンバーと退出メンバーのレポートを表示

**重要な制限事項:**
- 現在のバージョンでは、退出メンバーの検出のみ可能です
- メンバーの強制復帰にはOAuth2認証（guilds.joinスコープ）が必要です
- 実際の復帰機能を実装するには、各ユーザーのOAuth2アクセストークンが必要です
...
**権限:**
- `.env`ファイルの`ADMIN_USER_ID`で設定されたユーザーのみ実行可能
- デフォルト値: `568353089791328273`（変更可能）
